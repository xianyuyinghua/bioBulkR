enrich_go_kegg <- function(genes,
    ...
    sankeyplot <- function(data,
        ...
        p_final <- ggplot() +
            ...
            annotate("text",x = (bubble_box_xmin + bubble_box_xmax) / 2,y = axis_y - (3.5+(nrow(data)*0.2)), label = "Ratio",size = 8) + # x轴标题
    ...
    treeplot <- function(data,
        ...
        tree_data <- data %>% dplyr::transmute(ONTOLOGY = as.character(ONTOLOGY),
                                               Description = as.character(Description),
                                               geneID = as.character(geneID)) %>%
            dplyr::distinct() %>%
            dplyr::filter(!(ONTOLOGY == "KEGG" & Description == "KEGG")) %>% 
